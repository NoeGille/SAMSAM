...
def collate_fn(batch:list[tuple[dict, np.ndarray]]):
    ...
    return batched_data, batched_mask
...
def to_dict(img:np.ndarray, prompts:dict[str, np.ndarray], device:str='cuda') -> dict:
    ...
    if prompts['points'] is not None and prompts['neg_points'] is not None:
        ...
        point_labels = torch.ones(len(prompts['points'])).long().to(device)
        neg_point_coords = torch.tensor(prompts['neg_points']).float().to(device)
        neg_point_labels = torch.zeros(len(prompts['neg_points'])).long().to(device)
        ...
        output['point_labels'] = torch.ones(len(prompts['points'])).long().unsqueeze(0).to(device)
    elif prompts['neg_points'] is not None:
        output['point_coords'] = torch.tensor(prompts['neg_points']).float().unsqueeze(0).to(device)
        output['point_labels'] = torch.zeros(len(prompts['neg_points'])).long().unsqueeze(0).to(device)
    # box
